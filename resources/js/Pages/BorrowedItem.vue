  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center space-x-3">
        <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-xl flex items-center justify-center">
          <svg class="w-4 h-4 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
          </svg>
        </div>
        <div>
          <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Borrowed Items</h2>
          <!-- <p class="text-sm text-gray-600 mt-1 hidden sm:block">Manage and track all borrowed inventory items</p> -->
        </div>
      </div>
    </template>
    ...
    <div class="py-4 sm:py-8">
      <div class="mx-auto max-w-7xl px-3 sm:px-4 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
          <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <div class="flex items-center">
              <div class="p-1.5 sm:p-2 bg-blue-50 rounded-lg sm:rounded-xl">
              ...
              <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                ...
                <p class="text-lg sm:text-2xl font-bold text-gray-900">{{ sortedBorrowedItems.length }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <div class="flex items-center">
              <div class="p-1.5 sm:p-2 bg-amber-50 rounded-lg sm:rounded-xl">
              ...
              <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                ...
                <p class="text-lg sm:text-2xl font-bold text-gray-900">{{ sortedBorrowedItems.filter(item => (item.status || 'accepted').toLowerCase() === 'pending').length }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <div class="flex items-center">
              <div class="p-1.5 sm:p-2 bg-emerald-50 rounded-lg sm:rounded-xl">
              ...
              <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                ...
                <p class="text-lg sm:text-2xl font-bold text-gray-900">{{ sortedBorrowedItems.filter(item => (item.status || 'accepted').toLowerCase() === 'accepted').length }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <div class="flex items-center">
              <div class="p-1.5 sm:p-2 bg-sky-50 rounded-lg sm:rounded-xl">
                <svg class="w-4 h-4 sm:w-6 sm:h-6 text-sky-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                </svg>
              </div>
              <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Returned</p>
                <p class="text-lg sm:text-2xl font-bold text-gray-900">{{ sortedBorrowedItems.filter(item => (item.status || 'accepted').toLowerCase() === 'returned').length }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Main Content -->
        <div class="bg-white shadow-sm rounded-xl sm:rounded-2xl border border-gray-100 overflow-hidden">
          <div class="px-4 sm:px-6 py-4 sm:py-6 border-b border-gray-100">
            <div class="flex items-center justify-between">
              <h3 class="text-base sm:text-lg font-semibold text-gray-900">Items Overview</h3>
          ...
          <div class="block lg:hidden">
            <div class="divide-y divide-gray-100">
              <div 
                v-for="borrow in sortedBorrowedItems" 
                :key="borrow.id" 
                class="p-4 sm:p-6 hover:bg-gray-50/50 transition-all duration-200 active:bg-gray-100"
              >
                <!-- Item Header -->
                <div class="flex items-start justify-between mb-4">
                  <div class="flex items-center space-x-3 flex-1 min-w-0">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-xl flex items-center justify-center flex-shrink-0">
                  ...
                  <span
                    class="inline-flex items-center px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg text-xs font-semibold border flex-shrink-0 ml-2"
                    :class="statusClass(borrow.status)"
                  >
                    <span class="mr-1.5">{{ statusIcon(borrow.status) }}</span>
                    {{ (borrow.status || 'accepted').charAt(0).toUpperCase() + (borrow.status || 'accepted').slice(1) }}
                  </span>
                </div>
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2 sm:space-x-3">
                      ...
                      <span class="text-xs sm:text-sm text-gray-500">Borrower</span>
                    </div>
                    <div class="flex items-center space-x-2">
                      <div class="w-6 h-6 sm:w-7 sm:h-7 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center flex-shrink-0">
                        <span class="text-xs font-semibold text-indigo-700">
                          {{ (borrow.user?.name || 'N/A').charAt(0).toUpperCase() }}
                        </span>
                      </div>
                  ...
                  <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2 sm:space-x-3">
                      ...
                      <span class="text-xs sm:text-sm text-gray-500">Return Date</span>
                    </div>
          ...
          >
            ...
            <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2">No borrowed items found</h3>
